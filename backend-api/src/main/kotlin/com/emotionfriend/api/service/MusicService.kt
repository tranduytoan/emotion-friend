...
class MusicService(private val repo: MusicRepository) {
    ...
    suspend fun getById(id: Int): MusicTrack = repo.getById(id) ?: throw NoSuchElementException("Track '$id' not found")
    suspend fun create(track: MusicTrack): MusicTrack = repo.create(track)
    suspend fun update(id: Int, track: MusicTrack): MusicTrack = repo.update(id, track) ?: throw NoSuchElementException("Track '$id' not found")
